...
Deploy a win32lob app to Intune. Works well on large apps because it checks/renews the token before each blob upload

Based on example code from MS added many win32Lob properties to the original code
...
param(
        [parameter(Mandatory = $false, HelpMessage = "Specify the folder to be deployed.")]
...
[parameter(Mandatory=$false)]
[string]$CompanyPortalFeaturedApp,
...
[string]$PrivacyURL
    ...
    elseif($EXE){
        ...
        $body.installExperience = @{"@odata.type" = "#microsoft.graph.win32LobAppInstallExperience"; "runAsAccount" = "$installExperience"; "deviceRestartBehavior" = $RestartBehavior};
	    $body.informationUrl = $null;
	    $body.isFeatured = $false;
        $body.minimumSupportedOperatingSystem = @{"v10_1607" = $true};
        ...
        if ($icon) {
            $li = @{"@odata.type" = "microsoft.graph.mimeContent"}
            $li.type = "String"
            $li.value = $icon
            $body.largeIcon = $li 
        }
        if ($CompanyPortalFeaturedApp) { $body.isFeatured = $CompanyPortalFeaturedApp}
        if ($Developer) { $body.developer = $Developer}
        if ($InfomationURL) { $body.informationUrl = $InfomationURL }
        if ($PrivacyURL) { $body.privacyInformationUrl = $PrivacyURL }
...
    [ValidateNotNullOrEmpty()]
	...
	try	{
        ...
        Test-SourceFile "$SourceFile" | Out-Null
        ...
        if((($Ext).contains("msi") -or ($Ext).contains("Msi")) -and (!$installCmdLine -or !$uninstallCmdLine)){
            ...
            -icon $icon -CompanyPortalFeaturedApp $CompanyPortalFeaturedApp -RestartBehavior $RestartBehavior `
            -Developer $Developer -Owner $Owner -Notes $Notes -InformationURL $InfomationURL -PrivacyURL $PrivacyURL 
		...
		UploadFileToAzureStorage $file.azureStorageUri "$IntuneWinFile" $fileUri | Out-Null

        # Need to Add removal of IntuneWin file
        $IntuneWinFolder = [System.IO.Path]::GetDirectoryName("$IntuneWinFile")
        Remove-Item "$IntuneWinFile" -Force | Out-Null

		# Commit the file.
        Write-Host
        Write-Host "Committing the file into Azure Storage..." -ForegroundColor Yellow
		$commitFileUri = "mobileApps/$appId/$LOBType/contentVersions/$contentVersionId/files/$fileId/commit";
		MakePostRequest $commitFileUri ($fileEncryptionInfo | ConvertTo-Json) | Out-Null
		...
		MakePatchRequest $commitAppUri ($commitAppBody | ConvertTo-Json) | Out-Null

        Write-Host "Sleeping for $sleep seconds to allow patch completion..." -f Magenta
        Start-Sleep $sleep | Out-Null
        Write-Host
...
Function Build-Command2{
    # For Upload-Win32Lob 
    if (-not $ReturnCode) {
        $ReturnCode = Get-DefaultReturnCodes
    }

    <#
    foreach($key in $RequirementRule.Keys) {
        Write-Host "*** $key"
        foreach ($subKey in $RequirementRule[$key]) {
            Write-Host "   +++$subKey"
        }
    }
    #>
...
#$Win32Folder = "win32LobApp_BlueZzz_1.20.25"
. ./lib/Functions.ps1
Test-AuthToken
...
Try {
    ...
    if (-not $appExists) {
        $cmdParams = Build-Command2
        $app = Upload-Win32Lob @cmdParams
        Get-Token
        if ($DisplayVersion) {
    ...
    if (Test-Path "detection.xml") {Remove-Item "detection.xml" | Out-Null}
    Set-Location $scriptpath
}



<#  Original example main code ###
Test-AuthToken
...
$SourceFile = "C:\Users\MichaelVuozzo\Documents\github\pcatnewco-win32apps\win32LobApp_Nuance Dragon Naturally Speaking Professional_15\Nuance_DragonNaturallySpeakingProfessional_15.intunewin"
...
$MSIRule = New-DetectionRule -MSI -MSIproductCode "{768AC460-237E-45B6-99B8-1DE6D0F391E8}"
